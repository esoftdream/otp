...
class OTP
{
    ...
    public function generate(): array
    {
        ...
        $now = Time::now(config(\Config\App::class)->appTimezone);
    ...
    {
        $now = Time::now(config(\Config\App::class)->appTimezone)->toDateTimeString();
    ...
    public function verify(string $otpInput): bool
    {
        ...
        foreach ($otpRecords as $data) {
            ...
            if (password_verify($otpInput, $data->otp_value)) {
                ...
                $expiredAt = Time::parse($data->otp_expired_datetime, config(\Config\App::class)->appTimezone);
